...
function sumOfCodes(str) {
  let res = 0;
  if (typeof str !== 'string' || str instanceof String) return res;
...
function invertCase(str) {
  ...
  return str
    .split('')
    .map((char) =>
      uppercaseLetters.test(char) ? char.toLowerCase() : char.toUpperCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((char) => rot13.charAt(alph.indexOf(char)))
    .join('');
}
